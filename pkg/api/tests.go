...
func PrintTestsJSONFromDB(release string, w http.ResponseWriter, req *http.Request, dbc *db.DB) {
	...
	testsResult, err := BuildTestsResults(dbc, release, period, false, fil)
	if err != nil {
...
func BuildTestsResults(dbc *db.DB, release, period string, collapse bool, fil *filter.Filter) (testsAPIResult, error) {
	now := time.Now()
	...
	if collapse {
		rawQuery = rawQuery.Select(`name,
           ...
           sum(previous_flakes)    AS previous_flakes`).Group("name")
	}
