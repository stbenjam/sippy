	...
	for release, testIDOpts := range releaseToTestIDOptions {
		...
		go func() {
			...
			case <-ctx.Done():
				...
				return
			default:
				// Create modified reqOptions with the fallback release and test IDs
				fallbackReqOpts := r.reqOptions
				fallbackReqOpts.BaseRelease = reqopts.Release{
					Name:  release,
					Start: *start,
					End:   *end,
				}
				...
				jobRunTestStatus, tdErrs := r.dataProvider.QueryBaseJobRunTestStatus(ctx, fallbackReqOpts, allJobVariants)

				for _, err := range tdErrs {
					errCh <- err
				}
				...
				for jobName, rows := range jobRunTestStatus {
				...
				r.log.Infof("queried fallback base override job run test status: %d jobs, %d errors", len(r.baseOverrideStatus), len(tdErrs))
			}
		}()
	}
...
type fallbackTestQueryReleasesGeneratorCacheKey struct {
	...
	// CRTimeRoundingFactor is used by QueryReleaseDates
	CRTimeRoundingFactor time.Duration
	// KeyTestNames affects the query results via filtering logic
	KeyTestNames []string
}
	...
	for _, crRelease := range selectedTimeRanges {
		...
		go func(queryRelease crtest.ReleaseTimeRange, queryStart, queryEnd time.Time) {
			...
			default:
				...
				f.updateTestStatuses(queryRelease, stats)
			}
		}(*crRelease, start, end)
	}
	wg.Wait()
...
func (f *fallbackTestQueryReleasesGenerator) getTestFallbackRelease(ctx context.Context, release string, start, end time.Time) (map[string]crstatus.TestStatus, []error) {
	// Create modified reqOptions with the fallback release parameters
	...
	fallbackReqOpts.CacheOption = cache.RequestOptions{
		ForceRefresh:         false,
		CRTimeRoundingFactor: fallbackQueryTimeRoundingOverride,
	}
	...
	log.Infof("Fallback (%s) QueryTestStatus completed in %s with %d base results", release, time.Since(before), len(baseStatus))
