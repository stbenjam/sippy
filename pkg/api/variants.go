...
func listJobsByVariant(variant string, current, previous []v1.JobResult) []sippyv1.Job {
...
func PrintVariantsReport(w http.ResponseWriter, variant string, current, previous []v1.JobResult) {
	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	w.Header().Set("Access-Control-Allow-Origin", "*")
	response := listJobsByVariant(variant, current, previous)
